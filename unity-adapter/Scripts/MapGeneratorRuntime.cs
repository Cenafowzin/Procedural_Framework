...
///
/// Setup:
///   - Coloque mapgen.exe em  Assets/StreamingAssets/MapGen/mapgen.exe
///   - Instale Newtonsoft JSON: Package Manager → com.unity.nuget.newtonsoft-json
///   - Arraste o arquivo cornfield_pipeline.json como TextAsset no Inspector
///   - Configure o TileRegistry e o MapBuilder na cena
/// </summary>
public class MapGeneratorRuntime : MonoBehaviour
{
    ...
    private string RunMapgen(string configJson)
    {
        ...
        if (!File.Exists(path))
            throw new FileNotFoundException($"Binário não encontrado: {path}");
